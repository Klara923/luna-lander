...
function updateLayout() {
  gameScale = Math.min(windowWidth / GW, windowHeight / GH);
  offsetX = (windowWidth - GW * gameScale) / 2;
  offsetY = (windowHeight - GH * gameScale) / 2;

  if (button) {
    button.position(offsetX + 270 * gameScale, offsetY + 450 * gameScale);
    button.size(100 * gameScale, 30 * gameScale);
  }
  if (button1) {
    button1.position(offsetX + 265 * gameScale, offsetY + 420 * gameScale);
    button1.size(100 * gameScale, 30 * gameScale);
  }
}
